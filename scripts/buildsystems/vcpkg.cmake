...
macro(${VCPKG_OVERRIDE_FIND_PACKAGE_NAME} name)
    ...
    elseif("${name}" STREQUAL "Protobuf")
        _find_package(${ARGV})
        if(Protobuf_FOUND)
            if(NOT ENV{VCPKG_DEFAULT_TRIPLET} STREQUAL VCPKG_TARGET_TRIPLET)
                get_target_property(_vcpkg_protoc_exe_path protobuf::protoc IMPORTED_LOCATION)
		        string(REPLACE "/${VCPKG_TARGET_TRIPLET}/" "/$ENV{VCPKG_DEFAULT_TRIPLET}/" _vcpkg_protoc_exe_path "${_vcpkg_protoc_exe_path}")
                if(NOT EXISTS "${_vcpkg_protoc_exe_path}")
		            message(WARNING "Protobuf default triplet is not installed. Cross compile might fail. Try to install protobuf:$ENV{VCPKG_DEFAULT_TRIPLET} if you hit errors.")
                else()
                    set_target_properties(protobuf::protoc PROPERTIES IMPORTED_LOCATION "${_vcpkg_protoc_exe_path}")
                endif()
            endif()
        endif()
